...
class ContactFormManager {
  ...
  validateForm(data) {
    ...
    if (!recaptchaResponse) {
        this.showError('Το reCAPTCHA δεν φορτώθηκε σωστά');
        return false;
    }
...
// reCAPTCHA Responsive Manager
class RecaptchaResponsiveManager {
    constructor() {
        this.recaptcha = null;
        this.init();
    }
    ...
    init() {
        const checkInterval = setInterval(() => {
            this.recaptcha = document.querySelector('.g-recaptcha iframe');
            if (this.recaptcha) {
    ...
    scaleRecaptcha() {
        ...
        console.log(`reCAPTCHA scaled to ${scale} for width ${width}px`);
    }
}
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        new RecaptchaResponsiveManager();
